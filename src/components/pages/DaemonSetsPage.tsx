...
export default function DaemonSetsPage({ namespace }: DaemonSetsPageProps) {
  ...
  const [selectedDaemonSet, setSelectedDaemonSet] = useState<DaemonSetDetail | null>(null);
  ...
  const handleCreateDaemonSet = () => {
    ...
    createDaemonSet.mutate(
      ...
      {
        onSuccess: () => {
          toast({ title: "创建成功", description: `DaemonSet ${createForm.name} 已创建` });
          setIsCreateOpen(false);
          setCreateForm({ name: "", image: "", namespace: "default", port: 80 });
        },
        onError: (error) => {
          toast({
            title: "创建失败",
  ...
  // Handle delete DaemonSet
  const handleDelete = (dsNamespace: string, dsName: string) => {
    if (confirm(`确定要删除 DaemonSet "${dsName}" 吗？这将删除所有关联的 Pod。`)) {
  ...
  // Handle view DaemonSet detail
  const handleViewDetail = async (dsNamespace: string, dsName: string) => {
    try {
      const response = await fetch(`/api/daemonsets/detail?namespace=${dsNamespace}&name=${dsName}&XTransformPort=8080`);
      if (response.ok) {
        const data = await response.json();
        setSelectedDaemonSet(data);
  ...
  // Handle edit DaemonSet
  const handleEdit = (dsNamespace: string, dsName: string) => {
    setSelectedResource({ namespace: dsNamespace, name: dsName });
    setIsEditOpen(true);
  };
                    ...
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end" className="bg-slate-900 border-slate-700">
                        ...
                        <DropdownMenuItem 
                          ...
                          onClick={() => handleViewDetail(ds.namespace, ds.name)}
                        >
                          <Eye className="h-4 w-4 mr-2" /> 查看详情
                        </DropdownMenuItem>
                        <DropdownMenuItem 
                          className="text-slate-300 hover:text-white focus:bg-slate-800"
                          onClick={() => handleEdit(ds.namespace, ds.name)}
                        >
      ...
      <Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
        <DialogContent className="sm:max-w-[500px] bg-slate-900 border-slate-700">
          <DialogHeader>
            ...
            <DialogDescription className="text-slate-400">创建守护进程，确保每个节点上运行一个 Pod 副本</DialogDescription>
          </DialogHeader>
        ...
        <DialogContent className="max-w-5xl h-[700px] bg-slate-900 border-slate-700 flex flex-col">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 text-white">
              ...
              编辑 DaemonSet YAML - {selectedResource?.name}
            </DialogTitle>
            <DialogDescription className="text-slate-400">命名空间: {selectedResource?.namespace}</DialogDescription>
          </DialogHeader>
          {selectedResource && <ResourceYamlEditor kind="DaemonSet" namespace={selectedResource.namespace} name={selectedResource.name} onClose={() => setIsEditOpen(false)} />}
        </DialogContent>
      </Dialog>
            ...
            <div className="space-y-4 py-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-white font-mono">{selectedDaemonSet.name}</p>
                </div>
                <div>
                  <p className="text-slate-500 text-xs">命名空间</p>
                  <p className="text-white">{selectedDaemonSet.namespace}</p>
                </div>
                <div>
                  <p className="text-slate-500 text-xs">期望 Pod 数</p>
                  <p className="text-white">{selectedDaemonSet.desiredNodes}</p>
                </div>
                <div>
                  <p className="text-slate-500 text-xs">就绪</p>
                  <p className="text-emerald-400">{selectedDaemonSet.readyNodes}/{selectedDaemonSet.desiredNodes}</p>
                </div>
                <div className="col-span-2">
                  <p className="text-slate-500 text-xs">创建时间</p>
                  <p className="text-white text-sm">{selectedDaemonSet.createdAt}</p>
                </div>
              </div>
              {selectedDaemonSet.containers && selectedDaemonSet.containers.length > 0 && (
                <div>
