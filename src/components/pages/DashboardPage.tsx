...
const generateTimeData = (timeRange: string) => {
  ...
  const formatTime = (date: Date): string => {
    ...
    if (timeRange === "1m" || timeRange === "3m" || timeRange === "5m") {
  ...
  for (let i = points - 1; i >= 0; i--) {
    ...
    const memory = Math.floor(60 + Math.random() * 25);
    data.push({
...
export default function DashboardPage({ onNavigate }: DashboardPageProps) {
  ...
  const [metricType, setMetricType] = useState<"cpu" | "memory">("memory");
  const [chartData, setChartData] = useState(() => generateTimeData("24h"));

  const isLoading = statusLoading || overviewLoading;
  
  // 更新图表数据
  useEffect(() => {
    setChartData(generateTimeData(timeRange));
  }, [timeRange]);
            ...
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={chartData}>
                ...
                <Area
                  type="monotone"
                  dataKey={metricType}
                  stroke={metricType === "cpu" ? "#38bdf8" : "#a855f7"}
                  strokeWidth={2}
                  fill={metricType === "cpu" ? "url(#cpuGradient)" : "url(#memGradient)"}
                />
              </AreaChart>
            </ResponsiveContainer>
          </div>
